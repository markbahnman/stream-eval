...
class Summer extends _stream.Transform {
  ...
  _transform(chunk, encoding, done) {
    if (chunk !== null) {
      this._data = chunk;
      let output = `Bytes processed: ${(0, _utils.hSize)(chunk.size)} Lines: ${chunk.lines} Time elapsed: ${(0, _utils.hTime)(chunk.time)} Throughput: ${(0, _utils.hThroughput)(chunk.size, chunk.time)}`;
